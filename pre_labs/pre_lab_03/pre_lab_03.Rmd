...
It says  "Warning: One or more parsing issues, see 'problems ()' for details
...
**Answer** It gave me an error when I tried to run the lines.
```{r}
ppp_maryland_loans %>%
...
**Answer** It  gave me a similar error "could not find function "%>%""
```{r}
...
**Answer**
Same error as previous two answers
...
wrong data, missing data, or gaps in the data
...
It just added the letter "x" before the "1id" part
...
It's saying invalid "'type' (character) of argument."I assume that might mean it doesn't realize those are numbers that can be added together because it's registering them as titles or something.
```{r}
# cleaning function
...
There is one duplicate, and it is the Stuart Jones LLC.
